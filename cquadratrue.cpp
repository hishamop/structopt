...
CQuadrature::CQuadrature(int pts)
{
    m_pts =pts;
...
void CQuadrature::set_gauss_val(const double* x,const double* w)
{
...
double CQuadrature::AreaIntegral( double (*function)(double, double))
{
    double val=0;
...
/*
Eigen::MatrixXd CQuadrature::AreaIntegral(int, Eigen::MatrixXd (*function)(double, double))
{
    Eigen::MatrixXd val;
    val.setZero();
    for(int i=0;i<m_nodes.size();i++)
